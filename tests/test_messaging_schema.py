...
async def _make_user(db, *, email: str | None = None, slug: str | None = None) -> User:
    """Create a test user with the legacy single-key auth model.

    cueapi-core uses ``users.api_key_hash`` directly (not the multi-key
    ``api_keys`` table from private cueapi's migration 039 — that's
    HOSTED_ONLY for now). Tests here only need a User to attach Agents
    + Messages to; full auth flow is exercised in router-layer tests
    in PR-2.
    """
