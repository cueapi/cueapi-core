...
async def live_claim_execution(
    ...
    time when it wins the claim race. The server records
    ``live_claim_session_token`` + ``live_claimed_at`` on the
    execution row. The outcome validator then requires this
    attestation when a handler reports
    ``metadata.executed_via='live'`` — fabricated Live claims (e.g.
    BG-spawn ``claude --print`` subprocesses confabulating from
    auto-memory) are caught at outcome time because they can't forge
    an attestation written by the local claim-watcher.

    Write-once semantics: first attestation wins. A repeat call with
    the SAME ``session_token`` is a 200 idempotent ack. A repeat
    call with a DIFFERENT token returns 409 ``already_attested`` so
    the second caller knows they didn't win the attestation race.

    Parity port of cueapi/cueapi#664 (P0 Bulletproofing —
    cmotigtnx).
    """
...
async def _process_live_claim_request(
    *, db, user, execution_id, session_token: str
    ...
    ``"not_found" | "idempotent" | "conflict" | "fresh"`` and
    ``payload`` is ready to pass into either
    ``HTTPException(detail=...)`` or ``LiveClaimResponse(**...)``.

    Extracted from the endpoint so the per-branch logic is
    unit-testable via direct call (bypasses ASGI dispatch which
    pytest-cov on Python 3.12 doesn't trace cleanly — integration
    tests cover every branch but the patch-coverage tool reports
    them as uncovered without the helper).
    """
    result = await db.execute(
        select(Execution)
        .join(Cue, Execution.cue_id == Cue.id)
...
def _resolve_live_claim_attestation(
    *, execution, session_token: str, now: datetime
    ...
    """Pure helper: classify a live-claim attestation request and prep
    the response payload.

    Returns ``(outcome, payload)`` where ``outcome`` is one of:

    * ``"idempotent"`` — same session_token repeating an attestation
      that already exists on this execution. Caller returns 200 with
      the existing ``live_claimed_at``.
    * ``"conflict"`` — a *different* session_token was already
      attested. Caller raises 409 with the canonical CueAPI error
      envelope.
    * ``"fresh"`` — no prior attestation. The helper has set
    ...
    Pure so the per-branch logic is unit-testable without going
    through FastAPI + ASGI + DB.
    """
