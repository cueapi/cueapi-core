...
Spec: `https://trydock.ai/mike/cueapi-messaging-primitive-v1-sp` §6 (slug-form addressing) +
§13 D3 (lock-after-set semantics).
...
auto-derived value), then locked. v2 adds a separate aliasing primitive
(see §1.3a). v1 doesn't enforce the lock-after-set in this util — the
PATCH endpoint owns that policy.
