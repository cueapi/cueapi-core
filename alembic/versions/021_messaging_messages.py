...
* ``usage_messages_monthly`` table — per-user-per-month message
  ...
  from execution quotas (§13 D1 resolution).
* ``users.monthly_message_limit`` — per-plan quota, default 300 free,
  set higher by billing_service plan-tier config.

Spec reference: `https://trydock.ai/mike/cueapi-messaging-primitive-v1-sp` (in-repo canonical
once #457/#458 land) or
https://trydock.ai/mike/cueapi-messaging-primitive-v1-sp (historical).
...
def upgrade() -> None:
    ...
    # Default 300 matches free-tier execution-limit shape. Pro/scale
    # tiers updated by billing_service.PLAN_CONFIG via apply_plan_change.
    op.add_column(
        "users",
        sa.Column(
    ...
    op.create_table(
        ...
        # NOTE: ``from_api_key_id`` FK omitted — multi-key (api_keys
        # table) is HOSTED_ONLY. See models/agent.py for rationale.
        sa.Column(
            "to_agent_id",
            sa.String(length=20),
            sa.ForeignKey("agents.id", ondelete="SET NULL"),
        ...
        # (§3 R3 add). Empty default keeps the migration backfill-free
        # for any future-existing rows; no rows exist on first apply.
        sa.Column(
            "preview",
        ...
        # 30-day default TTL (§13 D7). Cleanup task transitions to
        # 'expired' once now() > expires_at. Hard-delete 7 days after
        # terminal state (§13 D10).
        sa.Column(
            "expires_at",
            sa.DateTime(timezone=True),
            nullable=False,
    ...
    # keys older than 24h to free for reuse — see §8.4.
    op.execute(sa.text("""
...
def downgrade() -> None:
    ...
    # Restore NOT NULL — only safe if no message-task rows exist.
    # Migration-up applies this on a system with no messages yet,
    # so down-migration on the same state is symmetric. If down-
    # migrating after messages have been delivered, the operator
    # must clean up message-task rows in dispatch_outbox first.
