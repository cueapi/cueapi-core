...
Spec reference: `https://trydock.ai/mike/cueapi-messaging-primitive-v1-sp` §2 (Identity primitive)
+ §6 (slug-form addressing). Authoritative source for review during
this migration's PR is the Dock workspace at
https://trydock.ai/mike/cueapi-messaging-primitive-v1-sp (frozen
2026-04-29) — once the freeze + decisions PRs (#457, #458) merge to
main, the in-repo file at https://trydock.ai/mike/cueapi-messaging-primitive-v1-sp is canonical.

Schema overview:

* ``agents`` — one row per addressable Identity. Format ``agt_<12 alphanum>``
  primary key (mirrors the existing ``cue_<12>`` convention from
  ``app/utils/ids.py``). Per-user scoping via ``user_id`` FK + optional
  per-key visibility via ``api_key_id`` FK (post-migration-039 pattern,
  same shape as ``cues.api_key_id``).
* ``users.slug`` — VARCHAR(64) per-tenant unique slug. Auto-derived
  ...
  ``PATCH /v1/auth/me`` subject to lock-after-set semantics (§13 D3).
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # NOTE: per-key visibility (``api_key_id`` FK to ``api_keys``) is
        # HOSTED_ONLY — cueapi-core uses the legacy single-key model.
        # See app/models/agent.py for the omission rationale.
        ...
        # cleanup task (mirrors api_keys revocation pattern).
    ...
    # this index speeds the addressing-resolver join in §6.2.
